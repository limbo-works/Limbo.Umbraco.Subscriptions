...
namespace Limbo.MailSystem.Templates.RazorTemplates.Builders {
    public class RazorBuilder : IRazorBuilder {
        ...
        public async Task<Mail> BuildMail(ICollection<Recipient> receivers, string subject, string viewPath, ITemplateModel templateModel) {
        ...
        public async Task<Mail> BuildMail(Sender from, ICollection<Recipient> receivers, string subject, string viewPath, ITemplateModel templateModel) {
            return new Mail(from, receivers, subject, await BuildMailBody(viewPath, templateModel));
        }

        public async Task<string> BuildMailBody(string viewPath, ITemplateModel templateModel) {
