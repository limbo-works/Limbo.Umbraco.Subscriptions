...
namespace Limbo.Subscriptions.Persistence.Subscribers.Models {
    public class Subscriber : GenericId {
        ...
        public static void Validate(Subscriber subscriber, bool checkRelations = true) {
            if (subscriber == null) throw new ArgumentException("Subscriber cannot be null", nameof(subscriber));
            if (subscriber.Name == null) throw new ArgumentException("Name cannot be null", nameof(subscriber));
            if (subscriber.Email == null) throw new ArgumentException("Email cannot be null", nameof(subscriber));
